...
the ibmmq package that links to the core MQ application interface.
...
* The error logger package may need to be explicitly downloaded
  ...
  On my system, I also had to forcibly download the logger package called
  from within the prometheus code, using `go get -u github.com/Sirupsen/logrus`.
